...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::connection('core')->create('pub.did_nft_links', function (Blueprint $table) {
            $table->uuid('id')->primary()->default(DB::raw('gen_random_uuid()'));
            $table->uuid('did_id')->index();
            $table->uuid('nft_id')->index();
            $table->string('nft_db_connection')->default('nfts');
            $table->string('role')->default('owner');
            $table->uuid('created_by')->nullable()->index();
            $table->timestamps();

            // Unique constraint
            $table->unique(['did_id', 'nft_id', 'role']);

            // Foreign key to did_profiles if schema supports it
            // Note: nft_id has no FK (cross-DB reference)
            try {
    ...
    {
        Schema::connection('core')->dropIfExists('pub.did_nft_links');
    }
};
