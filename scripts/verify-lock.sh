...
if [[ "${FEATURE_DID:-false}" == "true" ]]; then
    if [[ -f "$SCRIPT_DIR/verify-did.sh" ]]; then
        ...
        if is_container "${1:-}"; then
            # Running inside container: execute directly
            cd /var/www/html || exit 1
            FEATURE_DID=true IMDC_API_BASE_URL=http://web:80 ./scripts/verify-did.sh --in-container > /tmp/did_guardrail_output.txt 2>&1 || DID_EXIT=$?
        elif has_docker_compose; then
            # Running on host: use docker compose
            docker compose -f infra/docker/docker-compose.yml exec -T app sh -lc "cd /var/www/html && FEATURE_DID=true IMDC_API_BASE_URL=http://web:80 /var/www/html/scripts/verify-did.sh --in-container" > /tmp/did_guardrail_output.txt 2>&1 || DID_EXIT=$?
        else
            # No docker compose: try direct execution
            FEATURE_DID=true "$SCRIPT_DIR/verify-did.sh" > /tmp/did_guardrail_output.txt 2>&1 || DID_EXIT=$?
        fi
