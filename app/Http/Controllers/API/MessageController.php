...
class MessageController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($sid = $request->query('sender_id'))    $q->where('sender_id', (int)$sid);
        if ($rid = $request->query('receiver_id'))  $q->where('receiver_id', (int)$rid);
        if ($room = $request->query('safe_room_id'))$q->where('safe_room_id', (int)$room);
    ...
    public function destroy(int $id)
    {
        ...
        if ($deleted === 0) return ApiResponse::error('Message not found', 404);
