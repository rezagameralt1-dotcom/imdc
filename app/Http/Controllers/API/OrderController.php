...
class OrderController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($user = $request->query('user_id')) $q->where('user_id', (int)$user);
    ...
    public function addItem(Request $request, int $orderId)
    {
        ...
        if (!$exists) return ApiResponse::error('Order not found', 404);
    ...
    public function setStatus(Request $request, int $orderId)
    {
        ...
        if ($count === 0) return ApiResponse::error('Order not found', 404);
